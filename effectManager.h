...
public:
	...
	void addEffect(string effectName, const char* imageName, int imageWidth,
		int imageHeight, int effectWidth, int effectHeight, int fps, float elapsed, int buffer, bool isBlend = false);

	void play(string effectName, int x, int y, bool isBlend = false, BYTE alpha = 255);
};
