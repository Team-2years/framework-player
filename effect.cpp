...
{
	if (!_isRunning) return;
	RENDERMANAGER->push_BackEffectRenderInfo(_y + _effectImage->getHeight(), _effectAnimation, _effectImage, _x, _y, false, _isBlend, _alpha);
	// 이펙트를 출력하는 것이 였던것
	//_effectImage->aniRender(getMemDC(), _x, _y, _effectAnimation);
}

void effect::startEffect(int x, int y, bool isBlend, BYTE alpha)
{
